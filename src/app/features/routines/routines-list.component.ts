  ...
  template: `
    <div class="space-y-6">
      <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Rutinas</h1>
          ...
          @for (routine of filteredRoutines(); track routine.id) {
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 p-6">
              <div class="flex justify-between items-start mb-4">
                <div>
                  <h3 class="text-xl font-semibold text-gray-900 dark:text-white">{{ routine.name }}</h3>
                  <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ routine.description }}</p>
                </div>
